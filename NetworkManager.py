...
class NMDbusInterface(object):
    ...
    def __new__(klass, object_path=None):
        ...
        if object_path and not klass.introspection_data:
            proxy = dbus.SystemBus().get_object(klass.dbus_service, object_path, introspect=False)
    ...
    def proxy(self):
        if not self._proxy:
            self._proxy = dbus.SystemBus().get_object(self.dbus_service, self.object_path, introspect=False)
...
class ActiveConnection(TransientNMDbusInterface):
    ...
    def __new__(klass, object_path):
        if klass == ActiveConnection:
            ...
            obj = dbus.SystemBus().get_object(klass.dbus_service, object_path, introspect=False)
...
NM_CHECKPOINT_CREATE_FLAG_TRACK_INTERNAL_GLOBAL_DNS = 32
...
NM_ACTIVATION_STATE_FLAG_IS_CONTROLLER = 1
NM_ACTIVATION_STATE_FLAG_IS_PORT = 2
NM_ACTIVATION_STATE_FLAG_LAYER2_READY = 4
NM_ACTIVATION_STATE_FLAG_IP4_READY = 8
NM_ACTIVATION_STATE_FLAG_IP6_READY = 16
NM_ACTIVATION_STATE_FLAG_CONTROLLER_HAS_PORTS = 32
...
# Backward-compatible aliases for constants renamed in NM 1.48+
NM_ACTIVATION_STATE_FLAG_IS_MASTER = NM_ACTIVATION_STATE_FLAG_IS_CONTROLLER
NM_ACTIVATION_STATE_FLAG_IS_SLAVE = NM_ACTIVATION_STATE_FLAG_IS_PORT
NM_ACTIVATION_STATE_FLAG_MASTER_HAS_SLAVES = NM_ACTIVATION_STATE_FLAG_CONTROLLER_HAS_PORTS

# Legacy error constants removed in modern NM headers
